...
// Update ADMIN_USERS untuk hanya RW 01 dan 04 dengan ID unik per RW
const ADMIN_USERS = [
  {
    id: "admin-rw01",
    username: "admin",
...
export function AuthProvider({ children }: { children: ReactNode }) {
  ...
  // Update login function untuk handle RW selection yang lebih baik
  const login = async (username: string, password: string): Promise<boolean> => {
    // Check admin login
    if (username === "admin") {
      // Check if using default password
      if (password === DEFAULT_ADMIN_PASSWORD) {
        // Tampilkan pilihan RW
        ...
        if (adminUser) {
          // Check if this admin has already changed password
        ...
        // Try to find which RW admin this password belongs to
  ...
  const changePassword = async (
    ...
    if (user.role === "admin") {
      // Get current password for this specific admin
      const adminPasswords = JSON.parse(localStorage.getItem("admin_passwords") || "{}")
      ...
      // Save new password for this specific admin
